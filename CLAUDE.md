...
- All active/active_hidden listings in one sortable table: Listing (links to the live eBay item, sticky first column) | Price | Qty | Impressions | Views | CTR | 7-day Views sparkline
- **Impressions column shows both metrics as `narrow|all-sources`** (2026-08-09), value and delta both, e.g. `1,491|20,000` / `▼1%|▲2%` — maximizes information density without a second column. Sort still keys off all-sources. See Constraints for what narrow vs. all-sources means.
...
- Click a row to expand Impressions/Views/CTR detail charts with hover tooltips (tap-to-see on touch; no drag-scrub on mobile, see Constraints)
...
- **Mobile-optimized (2026-08-08/09)** — user views this on mobile >50% of the time. Sticky first column capped at 128px (not desktop's 340px) so scrolling actually reveals other columns; KPI tiles and detail charts stack to 1 column under 640px; detail chart sizing decoupled from the table's own 760px min-width (see Constraints, this one took 3 iterations to get right)
...
### 7. Takehome Calculator (`docs/takehome-calculator/`)
Static, client-side-only page (no backend, no data pipeline) — live at `pacificcards.github.io/ebay-tools/takehome-calculator/`. Finds the eBay listing price that nets the same takehome as a known Dealernet asking price (or vice versa), across each platform's fee scenarios. All math lives inline in `index.html`'s `<script>` block — no `.py` source, no build step.

**Freeform scenario columns, not a fixed 2-platform grid.** Each column is an independent "slip" cloned from one of 4 presets (Dealernet PayPal, Dealernet EFT, eBay Presale, eBay Same Day — default order, left to right) via the **+ Add scenario** button in the page header. Every column is fully self-contained (its own price, qty, and all fee/tax/shipping variables) and can be renamed or removed (**×**) independently; presets themselves are a fixed static list (`PRESETS` const) used only as clone templates, never mutated. A column's underlying fee formula ("family": `ebay` or `dealernet`) is fixed at clone time and drives both its field set and its anchor behavior.

**Anchor model:** typing a price in any column (or clicking any column's **Takehome** value, which re-anchors on that column's current price without changing anything else about it) makes it the anchor. Columns in the **same family** as the anchor mirror its exact unit price (shows how takehome differs by scenario at one price). Columns in the **other family** solve via bisection for the unit price that yields the *same takehome* as the anchor — this is the tool's actual purpose: starting from a known Dealernet price and backing into the equivalent eBay price (Dealernet PayPal defaults as the anchor on load).

**Fields, per column, in this order:** Unit Price + Qty (qty scales into the fee/tax math as `unitPrice × qty` before hitting the per-family formula) → Takehome (clickable, promoted directly under price) → fee breakdown (see below) → **basic** variables, always visible, in the same order across both families so they land on the same row: Tax %, Shipping Paid, Shipping Cost, then the family's scenario-differentiator (FVF Credit % for eBay / Fee Rate % for Dealernet), then List Discount % (eBay only — has no Dealernet counterpart, so it's trailing rather than leading to avoid pushing every shared field down a row) → **Advanced Fee Settings** (collapsed `<details>`, eBay only): FVF Rate %, Tier Threshold $, Low Tier Rate %, Flat Fee $, plus the itemized FVF derivation (see below). A **↻ sync** button next to Qty/Tax/Shipping Paid/Shipping Cost/List Discount copies that column's value for just that one field to every other live column (skips fields that don't exist on the target, e.g. syncing List Discount is a no-op on Dealernet columns); FVF Credit and Fee Rate are deliberately never syncable — they're what makes a scenario a scenario. Field notes (e.g. Dealernet's "tax cancels out of takehome") are hover/focus tooltips on a small "ⓘ", not inline text, so they don't add variable row height on one family but not the other.

**Fee breakdown**, directly under Takehome: `[Qty× line if qty≠1]` → `[List discount if >0]` → `[Shipping paid by buyer if >0]` → Gross Receipt → **Total eBay Fee** (single summary line; the itemized High tier/Low tier/Flat fee/FVF credit derivation that sums to it lives inside Advanced Fee Settings, recapped with its own Total eBay Fee line there) or Dealernet's single `Fee = (price+shipPaid)×rate` line → Tax → Shipping cost. Breakdown line count varies by family/scenario (conditional lines), so **all breakdown blocks are forced to the same height** (`syncBreakdownHeights()`, re-measures and re-applies after every recompute, plus on window resize) — otherwise the variable rows below would start at different vertical positions per column even with matching field order.

**eBay math:** `EffPrice = UnitPrice×Qty × (1 − Discount%)`; `Tax = (EffPrice + ShippingPaid) × Tax%`; FVF is tiered on eBay's real published rate — `(min(EffPrice, $2,500) + Tax + ShippingPaid) × 12.35%` plus `max(0, EffPrice − $2,500) × 2.35%` plus a flat $0.40 — **then a `FVF Credit %` is applied to that gross fee** (0% for Presale, 10% for Same Day by default): `fvfNet = fvfGross × (1 − Credit%)`. This was a real correction mid-session — Same Day and Presale are **not** two separately-quoted flat rates; they share one rate schedule, and Same Day's only difference is the credit. `Takehome = GrossReceipt − Tax − fvfNet − ShippingCost` (Tax inflates the FVF base but is subtracted back out before Takehome, since it's remitted, not kept).

**Dealernet math:** no discount/tiering — `Fee = (UnitPrice×Qty + ShippingPaid) × FeeRate%` (2.99% PayPal / 0% EFT default); `Takehome = GrossReceipt − Tax − Fee − ShippingCost`. Tax defaults to 0% and, like eBay, cancels out of Takehome algebraically — shown on the Gross Receipt line for symmetry with eBay, doesn't move the final number.
...
- **Traffic dashboard detail chart sizing on mobile — took 3 rounds to actually fix, all three lessons matter:**
  1. A CSS-only `width:100%; height:auto` on an svg with a `viewBox` depends on the browser correctly deriving height from that ratio when height is `auto` — inconsistent on mobile browsers specifically. Use `aspect-ratio: <w> / <h>` on the *wrapper* instead (`.chart-svg-wrap`) and give the svg `width:100%; height:100%` — deterministic everywhere.
  2. Never call `preventDefault()` on `touchmove` for a chart-scrub interaction. If the chart's hit-area is ever miscalculated (see #1 — it was, briefly), that `preventDefault()` blocks page scroll across the whole (oversized) hit-area, which reads to the user as the page freezing. Losing smooth drag-to-scrub on mobile is a much smaller cost than that.
  3. The detail panel is a `colspan` cell inside the *same* `<table>` as the main rows, and that table has `min-width:760px` for its 7 columns. Any percentage-width content inside the detail panel inherits that 760px+ width even on a 375px phone — this is what actually caused the oversized-chart symptom, not #1 alone. Fixed by giving `.detail-panel` `position: sticky; left: 0; width: calc(100vw - 24px)` in the mobile breakpoint, which decouples it from the table's width regardless of current horizontal scroll position. Any future content added inside the detail panel needs to respect this same constraint — don't assume the panel's container width equals the viewport without it.
  4. General lesson: a sticky/fixed-position element's coordinate math must never depend on absolute viewport coordinates from `getBoundingClientRect()` if there's any chance the page renders inside something that applies a `transform` to an ancestor (e.g. the Claude Artifact viewer) — that redefines the containing block for `position:fixed`. Prefer coordinates local to the nearest actually-positioned ancestor instead.
  5. Sticky first column at desktop's 340px width consumes nearly the entire screen on a phone, defeating the point of freezing it (nothing else is ever visible to scroll to). Capped at 128px under the mobile breakpoint.
...
### Takehome Calculator
- **Confirm the "Advanced Fee Settings" relocation matched intent** — user asked to "move the FVF calculator to the Advanced Fee Settings area"; interpreted as moving the itemized High tier/Low tier/Flat fee/FVF credit lines there (recapped with a Total eBay Fee line) while keeping a single Total eBay Fee summary in the main breakdown. Flagged as an interpretation in the handoff message; not yet explicitly confirmed by user.
- **No real-browser pixel verification** — this environment has no browser, so the breakdown height-sync and the Qty/Unit-Price box-height fix were verified structurally (jsdom: right elements exist, mechanism runs without error) rather than visually. Worked from user-supplied screenshots each round; worth a final look in an actual browser to confirm no remaining sub-pixel drift.
- No pending feature asks beyond the above as of 2026-08-12.
...
- Fixed the Qty input rendering ~1-2px shorter than the Unit Price input — they were built on different box models (Unit Price: padded/bordered wrapper around a borderless input; Qty: a directly padded/bordered input at a different font-size), so their heights only coincidentally nearly matched. Wrapped Qty in a `.qty-input-wrap` mirroring Unit Price's exact structure and gave both wrappers an explicit shared `height: 40px` with center alignment, so they're identical by construction.
- Both changes verified with jsdom (main breakdown no longer contains the relocated lines but the advanced panel's itemized sum matches the main summary exactly; Qty now shares Unit Price's wrapper class) plus a re-run of the FVF/credit math regression. See Open TODOs — no browser available in this environment to pixel-check the box-height fix visually; verification was structural only.

### 2026-08-10 — Takehome Calculator: new subproject built, then substantially reworked same day
- **MVP shipped:** static client-side page comparing equivalent listing price between eBay and Dealernet for the same net takehome (eBay: Same Day/Presale; Dealernet: PayPal/EFT). Spec gathered interactively — fee formulas, tiered-FVF mechanics, tax-in-takehome handling (included in FVF base, subtracted back out before Takehome since it's remitted not kept), Dealernet's flat-fee-on-(price+shipping) structure. Anchor/equivalence model iterated once before shipping: first draft solved all 3 other columns on any price edit, dropping the ask to see one eBay price's two different takehomes side by side; corrected to same-platform-mirrors / other-platform-solves. Verified via a standalone Node script (hand-derived tiered-FVF and boundary-continuity cases) plus a jsdom test of the actual shipped file.
- **Redesign** (user: "looks kinda horrendous") — moved off a generic gray-card look to a ledger/statement treatment: cool paper background, navy ink, cobalt (eBay)/teal (Dealernet) accents, monospace tabular-nums for every number, sharp-cornered cards, red-ink deduction lines. Follow-up trims same round: dropped the anchor/mirror/solved text chips (kept only a border highlight), simplified "Tax (remitted, not kept)" → "Tax", reordered Dealernet left of eBay with Dealernet defaulting as anchor once user named the actual primary workflow (starting from a known Dealernet price, solving for eBay).
- **Reworked into freeform scenario columns** — replaced the fixed 2-platform/2-scenario grid with independent, addable/removable columns cloned from 4 presets via a header "+ Add scenario" button; anchor/mirror/solve logic re-keyed off each column's fee-"family" (eBay-tiered vs Dealernet-flat) instead of a fixed platform grouping so it generalizes to any number of columns per family.
- **Added Unit Price (renamed from Sales Price) + Quantity**, qty scaling into the total fed to the per-family formula; **made every Takehome value a clickable button** that re-anchors on that column without touching any of its other variables (fell out of the existing anchor model almost for free); **added per-field ↻ sync buttons** (Qty, Tax%, Shipping Paid/Cost, List Discount) that copy one column's value for just that field across all live columns, skipping fields that don't exist on the target family.
- **Layout/decluttering pass:** promoted Takehome + the fee breakdown to sit directly under Unit Price/Qty (was buried after all variables); collapsed eBay's rarely-tweaked fields (FVF Rate, Tier Threshold, Low Tier Rate, Flat Fee) into a closed-by-default `<details>` reusing the Market Monitor dashboard's exact disclosure idiom; reordered fields so the ones shared between families (Tax/Shipping Paid/Shipping Cost) land on the same row across every column, pushing eBay-only List Discount to a trailing row; converted the two remaining inline field notes to hover-tooltip "ⓘ" icons since an always-visible note on one family's field but not the other's was silently throwing off row alignment even after reordering. Discovered and fixed a second, non-obvious alignment cause: Dealernet's shorter breakdown block was pushing the variable rows below it to a different height than eBay's, regardless of field order — added `syncBreakdownHeights()` to force all breakdown blocks to the tallest one, re-run after every recompute and on resize.
- **Added "Total eBay Fee" summary line** and conditional "List discount"/"Shipping paid by buyer" breakdown lines (shown only when non-zero), so both were previously baked silently into Gross Receipt with no line item explaining them.
- No backend/data pipeline for this subproject — pure static HTML/CSS/JS, all math + a generic bisection solver inline in `docs/takehome-calculator/index.html`'s `<script>` block, no `.py` source directory.
- Verification pattern used throughout every round: a Node script re-deriving hand-calculated values straight from the extracted math functions, plus a jsdom test actually running the shipped `<script>` and driving it through whatever interaction changed (typing, clicking, add/remove column) — caught at least one real arithmetic-adjacent bug this way (a test's own hand-calc omitting the tax-inflates-the-tier-base rule, not a code defect — traced and fixed the test).

### 2026-08-08 to 2026-08-09 — Mobile optimization pass and dual-impressions display
- Traffic Analytics: mobile pass on the traffic dashboard, prompted by user stating they view it on mobile >50% of the time. Found and fixed, in order as each was reported: (1) sticky first column at desktop's 340px width consuming the whole phone screen — capped at 128px under a new `@media (max-width:640px)` breakpoint, also added responsive stacking for the KPI row and detail-charts grid, a real horizontal-scroll fix for the Worst Performers mini-tables (they only had vertical overflow, columns were squeezing instead of scrolling), swipe-hint text, and touch support for chart tooltips; (2) detail charts rendering oversized and freezing page scroll — root cause was two-fold: `height:auto`-from-`viewBox` is inconsistent on mobile browsers, and the `touchmove` handler's `preventDefault()` (added for drag-scrub) blocked scrolling across whatever the oversized hit-area covered; fixed by sizing the chart wrapper via `aspect-ratio` instead and dropping `preventDefault()` from touch handling entirely; (3) even after that, charts were still ~2x too tall — the detail panel is a colspan cell inside the same `<table>` as the main rows (`min-width:760px` for 7 columns), so it was inheriting that width on a 375px screen; fixed with `position:sticky; left:0; width:calc(100vw - 24px)` on `.detail-panel`, scoped to mobile. All three fixes and the reasoning are in Constraints so this doesn't get re-broken.
- Traffic Analytics: user requested a systematic review after the first sticky-column fix ("this seems like a pretty simple thing that should be checked... review the rest of the dashboard") — swept the whole file for other fixed-pixel-width risks; found and fixed the detail-chart sizing bug above and added defensive `min-width:0` to `.chart-card`/`.kpi-tile` (CSS Grid items default to min-content sizing, which silently refuses to shrink below content size and can overflow the grid track).
- Traffic Analytics: added dual-impressions display — Impressions column now shows `narrow|all-sources` for both value and delta (e.g. `1,491|20,000` / `▼1%|▲2%`) instead of all-sources alone, maximizing information density without a second column. Mocked in the Artifact first with real narrow-impressions data pulled from Supabase (confirmed the narrow metric's history goes back further than all-sources' 7/31 start, a nice side benefit), then ported to production including `generate_dashboard.py` (now selects `impressions_search_and_store` too). User asked to drop the header's hover-tooltip definition of narrow/all-sources ("I get it") — kept the compact `(Narrow|All)` label, removed the `title` attribute.
- Confirmed a recurring but harmless pattern: the automated pipeline (or another session's) `chore: ... snapshot` commits to `docs/traffic/traffic_data.json` land on origin between local edits and push, essentially every time. Resolution is always the same — rebase, regenerate the JSON fresh via `generate_dashboard.py` rather than attempting to merge the blob, `git add`, continue.
...
- Traffic Analytics: `impressions_search_and_store` remains fetched/stored (unchanged) but no longer feeds any derived metric
- Traffic Analytics: flagged but did not yet act on — `impressions_all_sources` has no history before 2026-07-31, so DoD/WoW % comparisons for CTR and Ord-1k will show blank in the email until the comparison window ages past that date or a manual backfill is run; added as `next.md` item 13
- Traffic Analytics: separately flagged (not yet investigated) — `views_total` (`LISTING_VIEWS_TOTAL`) follows the same `LISTING_*_TOTAL` naming pattern that turned out to underreport for impressions; not verified against Seller Hub or against the sum of its own 5 source-breakdown columns
- Migration for this run confirmed applied to Supabase before push; push-triggered `analytics-ingest.yml` run (id 30597459145) completed successfully
