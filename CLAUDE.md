...
Static, client-side-only page (no backend, no data pipeline) — live at `pacificcards.github.io/ebay-tools/takehome-calculator/`. Finds the equivalent listing price between eBay and Dealernet that nets the same takehome, across each platform's fee scenarios.

**Layout:** Two platform groups side by side (eBay: Same Day / Presale columns; Dealernet: PayPal / EFT columns), each platform's shared inputs (discount/tax/shipping/tier settings) shown once above its scenario columns. All fee %, tax, discount, and shipping fields are editable inline — no hardcoded fee table to update in code when rates change.

**Anchor model:** typing a price in any column makes it the anchor. Columns on the **same platform** as the anchor mirror that exact price (shows how takehome differs by fee scenario at one price — e.g. Same Day vs Presale). Columns on the **other platform** solve (via bisection, since eBay's tiered fee is piecewise) for the price that yields the *same takehome* as the anchor. Anchor/mirror/solved state is labeled per column.

**eBay math:** `EffPrice = SalesPrice × (1 − Discount%)`; Tax = `(EffPrice + ShippingPaid) × Tax%`; FVF is tiered — `min(EffPrice, threshold) + Tax + ShippingPaid` at the high rate (11.5% Same Day / 12.5% Presale default), the EffPrice excess over the $2,500 threshold at a low rate (2.5% default — user's own spec; eBay's public rate is 2.35%, edit the field if it should match), plus a flat $0.40; Takehome = GrossReceipt − Tax − FVF − ShippingCost (tax is included in the FVF base but subtracted back out before Takehome, since it's remitted, not kept).

**Dealernet math:** simpler, no discount/tiering — fee = `(SalesPrice + ShippingPaid) × FeeRate%` (2.99% PayPal / 0% EFT default); Takehome = GrossReceipt − Tax − Fee − ShippingCost. Tax defaults to 0% and, like eBay, cancels out of Takehome algebraically (shown on the Gross Receipt line only, doesn't move the final number) — included per user request for symmetry with eBay, not because it does anything here.

**No backend/build step:** all four forward formulas + a generic bisection solver live inline in `docs/takehome-calculator/index.html`'s `<script>` block. Verified against hand-derived test cases (tiered-fee boundary continuity, round-trip eBay→Dealernet→eBay price recovery, tax-inertness on Dealernet) before shipping — see session log.
...
### 2026-08-10 — Takehome Calculator (new subproject, shipped)
- Built new subproject: static client-side page comparing equivalent listing price between eBay and Dealernet for the same net takehome, across each platform's fee scenarios (eBay: Same Day/Presale; Dealernet: PayPal/EFT). Live at `pacificcards.github.io/ebay-tools/takehome-calculator/`.
- Spec gathered interactively over several rounds — exact fee formulas, tiered-FVF mechanics (marginal split at $2,500, base = min(price,threshold)+tax+shipping at the high rate, excess at a low rate), tax-in-takehome handling (included in FVF base, subtracted back out before Takehome since it's remitted not kept), and Dealernet's simpler flat-fee-on-(price+shipping) structure with a payment-method scenario split (PayPal 2.99% vs EFT 0%).
- Design iterated once on the anchor/equivalence model: first draft had *any* price edit solve all three other columns, which silently dropped the user's explicit ask to see one eBay price's two different takehomes (Same Day vs Presale) side by side. Corrected to: same-platform columns as the anchor mirror its exact price (different takehome per scenario); other-platform columns solve for the price that matches the anchor's takehome (via bisection, since eBay's tiered fee is piecewise-linear, not closed-form-trivial to invert cleanly for a discount-adjusted price).
- Verified math two ways before shipping, not just "does it render": (1) a standalone Node script re-deriving hand-calculated expected values (tiered FVF above the $2,500 threshold, boundary continuity at the threshold — confirmed no discontinuity, just an expected slope kink — round-trip eBay→Dealernet→eBay price recovery, Dealernet's tax field being algebraically inert); (2) a full jsdom-driven test of the actual shipped HTML/JS (not a copy) — typed into each of the 4 price fields in turn, confirmed same-platform mirroring vs cross-platform solving, anchor label text, and that editing shared/scenario fields recomputes live without errors.
- No backend/data pipeline for this subproject — unlike the others, it's pure static HTML/CSS/JS (all 4 fee formulas + a generic bisection solver inline in one `<script>` block), so there's no `.py` source directory at repo root, just `docs/takehome-calculator/`.
- Per-column fee breakdown (gross receipt → high/low tier fee lines → tax remitted → shipping cost → takehome) is shown live under each column, not just the final number — makes the tiering assumption user-auditable against a real invoice if the numbers ever look off.
