...
# Add repo root (for shared.*) and listings-publisher (for images + ebay_api)
sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).parent.parent / "listings-publisher"))
...
def _build_aspects(row: dict) -> dict:
    aspects = {"Graded": ["No"]}
    if row.get("Game"):
        aspects["Game"] = [str(row["Game"])]
    if row.get("Language"):
        aspects["Language"] = [str(row["Language"])]
    if row.get("Character"):
        aspects["Character"] = [str(row["Character"])]
    if row.get("Card Number"):
        aspects["Card Number"] = [str(row["Card Number"])]
    if row.get("Rarity"):
        aspects["Rarity"] = [str(row["Rarity"])]
    if row.get("Finish"):
...
def _auto_title(row: dict) -> str:
    ...
    if row.get("Character"):
        parts.append(str(row["Character"]))
    if row.get("Card Number"):
        parts.append(str(row["Card Number"]))
    if row.get("Rarity"):
        parts.append(str(row["Rarity"]))
    game = str(row.get("Game", "Pokemon"))
    parts.append(f"{game} Card")
    return " ".join(parts)
...
def main(image_folder: str | None = None):
    ...
    sheet_id = _require("POKEMON_SHEET_ID")
    ...
    if listings:
        for row in listings:
            missing = [f for f in ("Character", "Game") if not row.get(f)]
            if missing:
    ...
    if listings:
        if not image_folder_str:
            ...
            print("  Usage: python pokemon-publisher/publish.py /path/to/photos")
            sys.exit(1)
