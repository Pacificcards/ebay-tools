"""Google Sheets read/write for the listings publisher."""
import gspread
...
# 1-based column positions matching the sheet layout
# A-N: listing fields, O-Q: policy IDs, R-V: package/offer fields, W-X: output
COL_SKU = 2
COL_LISTING_ID = 24
COL_STATUS = 25
...
def read_pending(sheet_id: str, creds_path: str) -> list[dict]:
    """Return rows that have a Player/Athlete but no Listing ID (not yet published)."""
    ws = _worksheet(sheet_id, creds_path)
    records = ws.get_all_records()
    pending = []
    for i, row in enumerate(records):
        if row.get("Player/Athlete") and not row.get("Listing ID"):
